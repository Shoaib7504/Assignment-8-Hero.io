...
const AppsDetails = () => {
    ...
    const handelAddToWishList = () => {
        ...
        let updatedList = []
        if (existingList) {
            const isDuplicate = existingList.some(p => p.id === apps.id)
            apps.quantity=apps.quantity +1
            if (isDuplicate) return alert('Your purchase has been confirmed!')
        ...
        <div>
            <div className='flex mt-20'>
                ...
                <div className='ml-20'>
                    ...
                    <button onClick={handelAddToWishList} className='mt-5 bg-green-400 text-white p-3 rounded-xl'>Install Now ({apps.size} MB)</button>
                </div>
